...
file(
    GLOB_RECURSE _ghostty_qt_all_resources
...
foreach(_ghostty_qt_resource IN LISTS _ghostty_qt_patched_resources)
    file(
        READ "${SHELL_INTEGRATION_DIR}/${_ghostty_qt_resource}"
        _ghostty_qt_resource_contents
    )
    string(
        FIND "${_ghostty_qt_resource_contents}"
        "ghostty-qt +ssh"
        _ghostty_qt_comment_position
    )
...
foreach(
    ...
    IN
    LISTS _ghostty_qt_expected_executable_references
)
    string(
        REPLACE "|"
        ";"
        _ghostty_qt_reference_parts
        "${_ghostty_qt_reference}"
    )
    ...
    string(
        FIND "${_ghostty_qt_resource_contents}"
        "${_ghostty_qt_expected_reference}"
        _ghostty_qt_reference_position
    )
...
foreach(_ghostty_qt_resource IN LISTS _ghostty_qt_all_resources)
    ...
    if(_ghostty_qt_resource_contents MATCHES "path join \"ghostty\"")
        message(
            FATAL_ERROR
            "${_ghostty_qt_resource} still calls the upstream ghostty executable"
        )
