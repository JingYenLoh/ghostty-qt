...
if(lock_result)
    message(
        FATAL_ERROR
        "Unable to lock Ghostty link matcher build: ${lock_result}"
    )
endif()
...
execute_process(
    COMMAND
        ...
        "-Dghostty-url-path=${URL_SOURCE}" "-Doptimize=${OPTIMIZE}" --prefix
...
if(NOT zig_result EQUAL 0)
    message(
        FATAL_ERROR
        "Pinned Ghostty link matcher build failed (${zig_result})"
    )
endif()
...
foreach(output_file "${MATCHER_LIBRARY}" "${ONIGURUMA_LIBRARY}")
    if(NOT EXISTS "${output_file}")
        message(
            FATAL_ERROR
            "Ghostty link matcher did not produce ${output_file}"
        )
    endif()
endforeach()
