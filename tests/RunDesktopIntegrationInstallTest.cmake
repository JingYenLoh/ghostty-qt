...
execute_process(
    COMMAND
        "${CMAKE_COMMAND}" -E env "DESTDIR=${staging_root}" "${CMAKE_COMMAND}"
        --install "${BUILD_DIR}" --prefix "${prefix}" --config "${CONFIG}"
...
file(
    COPY_FILE "${BUILD_DIR}/install_manifest.txt"
    "${staged_install_manifest}"
    ONLY_IF_DIFFERENT
)
...
foreach(
    ...
    if(NOT EXISTS "${required_path}")
        message(
            FATAL_ERROR
            "Installed desktop integration is missing: ${required_path}"
        )
    endif()
endforeach()
...
if(EXPECT_CONFIG_HELPER)
    if(
        NOT bash_completion_contents MATCHES "\\+show-config"
...
elseif(
    bash_completion_contents MATCHES "\\+show-config"
...
if(rsvg_convert)
    foreach(icon_size IN ITEMS 16 32 64 512)
        ...
        execute_process(
            COMMAND
                "${rsvg_convert}" --width "${icon_size}" --height "${icon_size}"
...
string(REPLACE [[\]] [[\\]] escaped_try_executable "${escaped_try_executable}")
...
foreach(
    quoted_character
    IN
    ITEMS "${desktop_double_quote}" "${desktop_dollar}" "${desktop_backtick}"
)
    string(CONCAT escaped_character [[\\]] "${quoted_character}")
    string(
        REPLACE "${quoted_character}"
        "${escaped_character}"
        escaped_dbus_executable
        "${escaped_dbus_executable}"
    )
...
string(
    FIND "${final_executable}"
    "${desktop_double_quote}"
...
set(desktop_exec "Exec=\"${escaped_executable}\" --single-instance=true")
set(desktop_try_exec "TryExec=${escaped_try_executable}")
string(
    CONCAT desktop_new_window_action
...
string(
    FIND "${desktop_contents}"
    "${desktop_new_window_action}"
    desktop_new_window_action_position
)
string(
    FIND "${service_contents}"
    "Name=${APPLICATION_ID}\n"
...
string(
    FIND "${systemd_service_contents}"
    "${systemd_service_exec}\n"
    systemd_service_exec_position
)

if(
    NOT desktop_contents MATCHES "(^|\n)DBusActivatable=true(\n|$)"
...
if(
    ...
    message(
        FATAL_ERROR
        "Invalid installed AppStream metadata:\n${metainfo_contents}"
    )
endif()

if(
    NOT icon_contents MATCHES "<svg"
...
if(
    systemd_service_exec_position EQUAL -1
    OR NOT systemd_service_contents MATCHES "(^|\n)Type=notify-reload(\n|$)"
    OR NOT systemd_service_contents MATCHES "(^|\n)ReloadSignal=SIGUSR2(\n|$)"
    OR NOT systemd_service_contents MATCHES "(^|\n)BusName=${APPLICATION_ID}(\n|$)"
    OR NOT systemd_service_contents
        MATCHES
        "(^|\n)WantedBy=graphical-session.target(\n|$)"
    OR systemd_service_contents
        MATCHES
        "__GHOSTTY_QT_INSTALL_(TRY_|DBUS_)?EXECUTABLE__|__GHOSTTY_QT_INSTALL_SYSTEMD_COMMAND__"
)
    message(
...
if(dbus_daemon AND gdbus AND kill_process)
    ...
    file(
        CREATE_LINK "${staging_root}${prefix}" "${prefix}"
        SYMBOLIC
    ...
    file(
        CHMOD "${activation_runtime}"
        PERMISSIONS OWNER_READ OWNER_WRITE OWNER_EXECUTE
    )
    file(
        WRITE "${activation_config}/ghostty/config.ghostty"
        "initial-window = true\nconfirm-close-surface = false\n"
    )
    file(
        WRITE "${activation_config}/ghostty-qt/config"
        "single-instance = false\n"
    )
    set(application_object_path "/${APPLICATION_ID}")
    string(REPLACE "." "/" application_object_path "${application_object_path}")
    string(REPLACE "-" "_" application_object_path "${application_object_path}")

    set(activation_address "unix:path=${activation_runtime}/bus")
    execute_process(
        COMMAND
            "${CMAKE_COMMAND}" -E env --unset=TERM_PROGRAM
            "XDG_DATA_HOME=${staging_root}${final_data_dir}"
            "XDG_CONFIG_HOME=${activation_config}"
            "XDG_RUNTIME_DIR=${activation_runtime}"
            "TMPDIR=${activation_runtime}" "GHOSTTY_QT_ALLOW_NON_WAYLAND=1"
            "GHOSTTY_QT_TEST_DESKTOP_ACTIVATION=1" "QT_QPA_PLATFORM=offscreen"
            "QT_QUICK_BACKEND=software" "${dbus_daemon}" --session
            "--address=${activation_address}" --fork --print-address=1
            --print-pid=1
    ...
    execute_process(
        COMMAND
            "${CMAKE_COMMAND}" -E env
            "DBUS_SESSION_BUS_ADDRESS=${activation_address}" "${gdbus}" call
            --session --dest "${APPLICATION_ID}" --object-path
            "${application_object_path}" --method
...
elseif(NOT EXPECT_CONFIG_HELPER AND EXISTS "${config_helper}")
    message(
        FATAL_ERROR
        "Config-disabled install unexpectedly contains ${config_helper}"
    )
endif()
...
execute_process(
    COMMAND
        "${CMAKE_COMMAND}" -E env "DESTDIR=${staging_root}" "${CMAKE_COMMAND}"
        "-DGHOSTTY_QT_INSTALL_MANIFEST=${staged_install_manifest}" -P
        "${uninstall_script}"
