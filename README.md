...
| `window-width`, `window-height` | Define the initial terminal grid in cells when both values are nonzero; one-sided values retain the ordinary Qt default. Ghostty's finalized 10-column by 4-row minimum is also used as the window's cell-derived resize minimum, bounded when even that minimum cannot fit the available screen. Every new window uses its resolved font size—including pane-inherited manual zoom—adds the persistent Qt toolbar, inherits the originating/focused window's screen when available, clamps best-effort to that screen, and seeds normal geometry before maximize/fullscreen presentation. Reload affects future windows only. The settled pre-map grid matches the requested cells when screen or compositor constraints do not reduce it, but the worker currently creates the PTY at 80×24 before its queued first resize, so a child can briefly observe that size during startup. |
