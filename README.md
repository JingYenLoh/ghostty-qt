...
- Mouse selection with libghostty-classified single-, double-, and triple-click
  ...
  behavior, a native-requested Qt context menu, and worker-authoritative
...
| `right-click-action` | Applies `context-menu`, `paste`, `copy`, `copy-or-paste`, or `ignore` live. Terminal mouse reporting wins unless `Shift` releases capture; raw DEC state still governs removal of that escape modifier when `mouse-reporting=false`. The worker resolves selection containment and copy-or-paste atomically, and rapid non-idempotent paste presses remain independent while only older popup presentations are superseded. Context clicks preserve an existing containing selection, otherwise select a qualifying `Ctrl` OSC 8 cell, full default URL/path match, or word before opening a `Popup.Native` Qt Quick menu with Copy and Paste. Stale frame coordinates still open the menu without selecting unrelated text. Copy output currently uses the plain representation pending Ghostty's exact public mixed-clipboard formatter. The initial menu omits Notify, Clear/Reset, Split, Tab, Window, and Config sections, and Shift currently always escapes raw capture because `mouse-shift-capture`/`XTSHIFTESCAPE` overrides are not yet implemented. |
